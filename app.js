  ...
  if (encounterId) {
    ...
    datastore
      ...
      .then((entity) => {
        if (entity) {
          debugLog(
            "Provider found existing encounter " +
        ...
        user.withCredentials(request, response, (client) => {
          calendar.createEvent(client, encounterId, (err, url) => {
            if (err) {
              debugLog(
                "ERROR: Provider calendar event create for encounter " +
                  request.body.encounterId +
                  " failed with error " +
                  err
              );
            ...
            debugLog(
              "Provider cfreated calendar event for encounter " +
                request.body.encounterId +
                " with URL " +
                url
            );
    ...
    user.withCredentials(request, response, (client) => {
      calendar.createEvent(client, "", (err, url) => {
        if (err) {
          debugLog(
            "ERROR: Provider calendar event without encounter failed with error " +
              err
          );
          response.status(500).send(err);
          return;
        ...
        debugLog(
          "Provider created calendar event without encounter with URL " + url
        );
        response.send({ url: url });
      });
    });
